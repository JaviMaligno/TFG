...
LA ÚLTIMA DE MONOIDE, EL RECÍPROCO ME LO DA LA PROPOSICIÓN ANTERIOR
...
\item $\beta_k(t)$ es monótona creciente en la segunda coordenada para todo $k$. 
\end{itemize}
...
\begin{prop}
Si un monoide satisface las condiciones de Ore, entonces se inyecta en su grupo de fracciones \cite{Luis}.
\end{prop}



DECK TRANSFORMATION -> TRANSFORMACIÓN RECUBRIDORA

DEFINIRLAS COMO AUTOMORFISMOS 
...
\begin{defi}
Sea $X$ un espacio topológico y $\widetilde{X}$ un espacio recubridor de $X$ con aplicación recubridora $p$. Dado un grupo $G$ actuando sobre cada fibra $p^{-1}(x)\subseteq\widetilde{X}$ con $x\in X$, si $p(gx)=p(x)$ para todo $g\in G$, se dice que $G$ es un grupo de \emph{transformaciones de recubrimiento} (\emph{deck transformations}).
\end{defi}
...
\begin{defi}
Dado un espacio topológico $X$ y un grupo $G$, se dice que un espacio recubridor $\widetilde{X}$ \emph{se corresponde} a $G$ si $\pi_1(\widetilde{X})=G$. 
\end{defi}
